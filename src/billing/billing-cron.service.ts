...
// Claves numéricas para pg_advisory_lock (una por cron)
const LOCK_RECURRING = 1001;
const LOCK_RETRY_FAILED = 1002;
const LOCK_RECONCILE = 1003;
const LOCK_RETRY_INVOICES = 1004;
...
export class BillingCronService {
  ...
  /**
   * Intenta adquirir un advisory lock en PostgreSQL.
   * Retorna el cliente si lo consiguió, o null si otra instancia ya lo tiene.
   * El caller debe liberar el cliente en finally (lo que libera el lock).
   */
  private async tryAcquireLock(key: number, name: string): Promise<PoolClient | null> {
    ...
    if (rows[0]?.lock) {
      this.logger.log(`Advisory lock acquired for ${name}`);
      return client;
    }
    client.release();
    this.logger.log(`Advisory lock NOT acquired for ${name}, skipping (another instance is running)`);
    return null;
  }

  @Cron('0 1 * * *')
  async processRecurringPayments(): Promise<void> {
    const lockClient = await this.tryAcquireLock(LOCK_RECURRING, 'processRecurringPayments');
    ...
    try {
      ...
      await lockClient.query(`SELECT pg_advisory_unlock($1)`, [LOCK_RECURRING]).catch(() => { });
      lockClient.release();
    }
  }
  ...
  async retryFailedPayments(): Promise<void> {
    const lockClient = await this.tryAcquireLock(LOCK_RETRY_FAILED, 'retryFailedPayments');
    if (!lockClient) return;
    ...
    try {
      ...
      await lockClient.query(`SELECT pg_advisory_unlock($1)`, [LOCK_RETRY_FAILED]).catch(() => { });
      lockClient.release();
    }
  }
  ...
  async reconcilePendingTransactions(): Promise<void> {
    const lockClient = await this.tryAcquireLock(LOCK_RECONCILE, 'reconcilePendingTransactions');
    if (!lockClient) return;
    ...
    try {
      ...
      await lockClient.query(`SELECT pg_advisory_unlock($1)`, [LOCK_RECONCILE]).catch(() => { });
      lockClient.release();
    }
  }
  ...
  async retryPendingInvoices(): Promise<void> {
    const lockClient = await this.tryAcquireLock(LOCK_RETRY_INVOICES, 'retryPendingInvoices');
    if (!lockClient) return;
    ...
    try {
      ...
      await lockClient.query(`SELECT pg_advisory_unlock($1)`, [LOCK_RETRY_INVOICES]).catch(() => { });
      lockClient.release();
    }
  }
